...
// Definitie zit in adnet_kb.cpp, zodat alle modules dezelfde queue delen.
#define KEY_BUFFER_SIZE 8
extern volatile uint8_t g_key_buffer[KEY_BUFFER_SIZE];
extern volatile uint8_t g_key_buffer_head;
extern volatile uint8_t g_key_buffer_tail;
...
void adamnet_inject_scancode(uint8_t sc);
void adamnet_set_game_mode(bool enabled);
bool adamnet_is_game_mode(void);
void adamnet_block_ascii_fkeys(int count);
void adamnet_host_prn_write_ascii(const char* s);

typedef void (*AdamPrinterSink)(const char* data, int len);
void adam_printer_set_sink(AdamPrinterSink sink);
