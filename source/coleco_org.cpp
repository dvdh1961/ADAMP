...
void coleco_WriteByte(unsigned int Address, int Data)
{
    ...
    else if (((emulator->currentMachineType != MACHINEADAM) && (emulator->typebackup==EEP24C08)) || (emulator->typebackup==EEP24C256) )
    {
        ...
        {
            // Boxxle selecteert welke 16K bank zichtbaar is in het 0xC000-0xFFFF gebied
...
int coleco_do_scanline(void)
{
     ...
     if (!emulator->stop && !emulator->singlestep)
    {
        ...
        do {
            ...
            // --- ADAMNET POLLING ---
            // Poll alle AdamNet devices om io_busy af te tellen
            // Dit moet elke frame gebeuren, niet alleen bij DCB reads!
            byte maxDCB = GetMaxDCB();
            if (maxDCB > 0 && maxDCB < 16) {
            for (byte dev = 0; dev <= maxDCB; dev++) {
                if(m_cpm_enabled)
                    if (!m_tdos_enabled)
                        UpdateDCB_CPM(dev, -1);
             }
