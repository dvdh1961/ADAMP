...
void CvBasicEditorWindow::setupDefaultPaths()
{
    const QDir appDir(QCoreApplication::applicationDirPath());

#if defined(Q_OS_WIN)
    m_cvbasicExePath = QDir::cleanPath(appDir.filePath("tools/cvbasic/cvbasic.exe"));
    m_gasm80ExePath  = QDir::cleanPath(appDir.filePath("tools/cvbasic/gasm80.exe"));
#else
    m_cvbasicExePath = QDir::cleanPath(appDir.filePath("tools/cvbasic/cvbasic"));
    m_gasm80ExePath  = QDir::cleanPath(appDir.filePath("tools/cvbasic/gasm80"));
#endif

    m_buildDirPath   = QDir::cleanPath(appDir.filePath("media/cvbasic/build"));
    m_lastOpenDir    = QDir::cleanPath(appDir.filePath("media/cvbasic/source"));
}
...
void CvBasicEditorWindow::loadSettings()
{
    QSettings s(QCoreApplication::applicationDirPath() + "/settings.ini", QSettings::IniFormat);
    s.beginGroup("cvbasic");
    m_cvbasicExePath = s.value("cvbasicExe", m_cvbasicExePath).toString();
    m_gasm80ExePath = s.value("gasm80Exe", m_gasm80ExePath).toString();
    m_buildDirPath = s.value("buildDir", m_buildDirPath).toString();
    m_lastOpenDir = s.value("lastOpenDir", m_lastOpenDir).toString();
...
void CvBasicEditorWindow::saveSettings()
{
    QSettings s(QCoreApplication::applicationDirPath() + "/settings.ini", QSettings::IniFormat);
    s.beginGroup("cvbasic");
    s.setValue("cvbasicExe", m_cvbasicExePath);
    s.setValue("gasm80Exe", m_gasm80ExePath);
    s.setValue("buildDir", m_buildDirPath);
...
void CvBasicEditorWindow::chooseCvBasicExe()
{
    ...
    if (!path.isEmpty()) {
        ...
        saveSettings();
        updateStatusText("cvbasic.exe set");
    }
}
...
void CvBasicEditorWindow::chooseGasm80Exe()
{
    ...
    if (!path.isEmpty()) {
        ...
        saveSettings();
        updateStatusText("gasm80.exe set");
    }
}
...
void CvBasicEditorWindow::startBuild(bool runAfterBuild)
{
    ...
    if (!ensureToolExists(m_cvbasicExePath, "cvbasic.exe"))
        return;

    if (!ensureToolExists(m_gasm80ExePath, "gasm80.exe"))
        return;
