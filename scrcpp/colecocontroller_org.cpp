...
static void releaseCpm80StateForPowerOff()
{
    /*
     * PowerOff must behave like a real power cycle.
     * If the user switched CP/M from 40C to F18A TERM80 manually,
     * clear every 80-column overlay/state before restarting Writer.
     */
...
QImage ColecoController::frameFromBridge()
{
    ...
    const int h = vb_current_height();

    QImage img(w, h, QImage::Format_ARGB32);

    for (int y = 0; y < h; ++y) {
        void *dstLine = img.scanLine(y);
        const void *srcLine = &g_video_frame[y * VB_MAX_WIDTH];
        std::memcpy(dstLine, srcLine, size_t(w) * sizeof(uint32_t));
    }
...
void ColecoController::loadDisk(int drive, const QString& path)
{
    ...
    m_tdos_enabled = isTdosDisk && !isCpmDisk;
...
void ColecoController::loadTape(int drive, const QString& path)
{
    ...
    m_tdos_enabled = isTdosTape && !isCpmTape;
